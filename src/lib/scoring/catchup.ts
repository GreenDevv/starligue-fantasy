...
// managers cette journée-là, × un facteur (défaut 0,7 : malus d'arrivée tardive —
// décourage d'attendre et compense l'avantage de constituer son effectif en
// connaissance de cause, sans condamner un arrivant qui a raté la deadline de peu).
// L'arrivant repart dans le tiers inférieur du barème médian sur les journées
// manquées, puis grimpe/descend ensuite à son mérite.

export interface CatchupConfig {
  enabled: boolean;
  factor: number; // CATCHUP_FACTOR — multiplie la médiane (défaut 0,7)
}
...
export function parseCatchupConfig(
  ...
  return {
    ...
    factor: parseFloat(raw["CATCHUP_FACTOR"] ?? "0.7"),
    ...overrides,
  };
}
