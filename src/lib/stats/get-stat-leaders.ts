// Calcul du top 5 d'une ligne de stat détaillée (src/lib/stats/stat-lines.ts) : accumulée
// depuis le début de la saison, pour une seule journée, ou en moyenne par match joué
...
export interface GetStatLeadersParams {
  statKey: string;
...
export interface GetStatLeadersResult {
  statKey: string;
  scope: "season" | "gameweek" | "average";
...
export async function getStatLeaders(params: GetStatLeadersParams): Promise<GetStatLeadersResult> {
  const { statKey, scope, seasonId } = params;
  ...
  let gameweekNumber = params.gameweekNumber ?? null;
  ...
  if (statKey === "fantasyPoints") {
    ...
    ranked = totals
      .filter((t) => (t._sum.shotsFaced ?? 0) > 0)
      .map((t) => ({
        playerId: t.playerId,
    ...
    // variable (template-literal type check "needs to be provided in by"), d'où le
    // passage par `any` en frontière d'appel plutôt que de dupliquer 13 branches. Tri
    // et top 5 faits en JS plutôt que via orderBy/take Prisma : Postgres trie NULLS
