...
// POST /api/trades/:id/reject — le destinataire refuse une proposition PENDING.
...
export async function POST(_request: Request, { params }: { params: { id: string } }) {
  ...
  const proposal =
    mode === "simulation"
      ? await prisma.simulationTradeProposal.findUnique({
          where: { id: params.id },
          include: { receivingTeam: { select: { userId: true } } },
        })
  ...
  if (proposal.receivingTeam.userId !== session.user.id) {
    return NextResponse.json(
      { error: { code: "FORBIDDEN", message: "Seul le destinataire peut refuser ce trade" } },
      { status: 403 }
    );
  }
  ...
  if (mode === "simulation") {
    await prisma.simulationTradeProposal.update({ where: { id: proposal.id }, data: { status: "REJECTED", respondedAt: now } });
  } else {
    await prisma.tradeProposal.update({ where: { id: proposal.id }, data: { status: "REJECTED", respondedAt: now } });
  }
