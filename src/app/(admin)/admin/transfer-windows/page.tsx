...
function formatRange(opensAt: string, closesAt: string): string {
      ...
      <div className="flex-1 overflow-y-auto p-5">
        <div className="flex flex-col gap-4">
          ...
          <p className="text-[10px] text-text-muted">
            Pour la simulation, l&apos;ouverture/fermeture est jugée par rapport à la
            progression de chaque équipe dans le calendrier historique (pas
            l&apos;horloge réelle) — voir src/lib/transfers/window.ts.
          </p>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-3xl uppercase tracking-wide text-text">Fenêtres de transfert</h1>
          <p className="mt-1 text-sm text-text-muted">
            {windows.length} fenêtre{windows.length !== 1 ? "s" : ""} — Noël + trêves internationales, live et simulation
          </p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-dashed border-border py-16 text-center text-sm text-text-muted">
          Aucune fenêtre de transfert. Crée-en une pour la saison live ou la simulation.
        </div>
      ) : (
            ...
            return (
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-text-muted">{formatRange(w.opensAt, w.closesAt)}</p>
                </div>
              </button>
            );
          })}
