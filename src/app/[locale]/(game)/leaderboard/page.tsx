  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="pixel-corners flex gap-1 border border-border bg-surface p-1">
        {(["global", "gameweek"] as const).map((tabKey) => (
          <button
            key={tabKey}
            onClick={() => { setTab(tabKey); setLoading(true); }}
            className={[
              "pixel-corners-sm flex-1 py-1.5 text-sm font-semibold uppercase tracking-wide transition-colors",
              tab === tabKey ? "bg-accent text-bg shadow-glow-accent" : "text-text-muted hover:text-text",
            ].join(" ")}
          >
            {tabKey === "global" ? t("tabs.global") : t("tabs.gameweek")}
          </button>
        ))}
      </div>
      ...
      {tab === "global" && (
        <div className="flex flex-col gap-3">
          {loading ? (
            <div className="py-10 text-center text-text-muted">{t("loading")}</div>
          ) : (
            <>
              {globalData && (
                <>
                  <p className="text-xs text-text-muted">
                    {t("totalTeams", { count: globalData.total })}
                  </p>
                  <LeaderboardList
          ...
          {gwNumber !== null && (
            <div className="flex items-center justify-center gap-4">
              ...
              <span className="font-arcade text-lg tracking-wide text-text">
                {t("gameweekLabel", { number: gwNumber })}
              </span>
              <button
