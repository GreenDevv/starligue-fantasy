...
export default async function ClubPage({ params }: { params: { id: string } }) {
  ...
  // Up/Coupe de France/EHF) — demande explicite de l'utilisateur, voir
  // getLastFiveForm (club-page-data.ts) pour la règle anti-spoiler simulation.
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      >
        ...
        {/* Rang + V/N/D + forme Starligue du club — demande explicite de
            l'utilisateur ("en jaune, en assez gros"), même palette/police que les
            autres valeurs numériques mises en avant (points, prix, deadline —
            ARCHITECTURE.md §8.1 : accent-secondary + police arcade réservées à cet
            usage). Sur la droite pour équilibrer avec le logo à gauche (demande
            explicite). */}
        {clubStanding && (
          <ClubFormBadge
            rank={clubStanding.rank}
            wins={clubStanding.wins}
            draws={clubStanding.draws}
            losses={clubStanding.losses}
            lastFive={lastFive}
            className="shrink-0"
          />
        )}
