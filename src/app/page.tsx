  ...
  if (!season) {
    return (
      <main className="mx-auto flex min-h-screen max-w-3xl flex-col items-center justify-center px-6 text-center">
        <p className="text-text-muted">Aucune saison en cours pour l&apos;instant.</p>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-6xl flex-col gap-4 px-4 pb-16 pt-6 sm:px-6">
      <div className="flex items-start justify-between gap-3">
        <div className="flex flex-col gap-1">
          <p className="font-arcade text-sm uppercase tracking-[0.3em] text-accent-secondary">
            Daikin Starligue · {season.label}
          </p>
          <h1 className="font-display text-3xl uppercase tracking-wide text-text">Starligue</h1>
        </div>
      ...
      <div className="pixel-corners flex flex-col items-start gap-2 border border-border bg-surface p-4 sm:flex-row sm:items-center sm:justify-between">
        {session?.user ? (
          <p className="text-sm text-text">
            Bon retour, <span className="font-semibold">{session.user.name}</span> !
          </p>
        ) : (
          <p className="text-sm text-text">
            Bienvenue — résultats, classement et actus de la Daikin StarLigue en direct.
          </p>
        )}
        <Link
        ...
        >
          {session?.user ? "Accéder à Fantasy Starligue →" : "Créer mon équipe Fantasy →"}
        </Link>
      </div>
      ...
      <p className="pt-2 text-center text-[11px] text-text-muted/60">
        Projet perso, sans rapport officiel avec la LNH ou la Daikin StarLigue. ·{" "}
