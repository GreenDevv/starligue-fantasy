...
export default async function LineupDetailPage({
  ...
  const bonusLabel = lineup.bonus ? BONUS_LABELS[lineup.bonus] : null;
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl text-text">Journée {lineup.gameweek.number}</h1>
          <p className="mt-1 text-sm text-text-muted">
            {mode === "simulation"
              ? isScored ? "Journée calculée" : "Pas encore calculée"
              : isScored ? "Journée scorée" : "En attente des notes"}
          </p>
          {bonusLabel && (
            <p className="mt-1 text-xs font-semibold uppercase tracking-widest text-accent-secondary">
              Bonus actif · {bonusLabel}
            </p>
          )}
        </div>
        {totalPoints !== null && (
