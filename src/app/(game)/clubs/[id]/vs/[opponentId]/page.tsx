...
const STAT_ROWS: { key: keyof ClubStandingRow; label: string }[] = [
  ...
  { key: "goalsFor", label: "Buts pour" },
  { key: "goalsAgainst", label: "Buts contre" },
  { key: "goalAvg", label: "Diff." },
];
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-center gap-4 pixel-corners border border-border bg-surface p-4">
        ...
        <span className="font-display text-lg uppercase tracking-widest text-text-muted">VS</span>
      ...
      <div>
        <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-text-muted">Saison en cours</p>
        {!standingA || !standingB ? (
          <div className="pixel-corners border border-border bg-surface px-4 py-6 text-center">
            <p className="text-sm text-text-muted">Classement pas encore disponible.</p>
          </div>
        ) : (
      ...
      <div>
        <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-text-muted">
          Confrontations directes {h2h.length > 0 && `— ${h2h.length} dernière${h2h.length > 1 ? "s" : ""}`}
        </p>
        {h2h.length === 0 ? (
          <div className="pixel-corners border border-border bg-surface px-4 py-6 text-center">
            <p className="text-sm text-text-muted">Aucune confrontation trouvée entre ces deux clubs.</p>
          </div>
        ) : (
          <>
            <p className="mb-2 text-center text-xs text-text-muted">
              {winsA} victoire{winsA > 1 ? "s" : ""} {clubA.shortName} · {draws} nul{draws > 1 ? "s" : ""} · {winsB} victoire
              {winsB > 1 ? "s" : ""} {clubB.shortName}
            </p>
            <div className="overflow-hidden pixel-corners border border-border bg-surface">
              <div className="divide-y divide-border">
                {h2h.map((m, i) => {
